...
public class Candidates extends BasePage {
	...
	public boolean isVacancyFiltered(String vacancy) {
		...
		for (int i = 1; i <= rows.size(); i++) {
			String dynamicXpath = "//div[@class='oxd-table-body']/div["+i+"]//div[contains(@class,'oxd-table-cell')]//descendant::div[contains(text(),'"+vacancy+"')]";
			try {
	...
	public boolean isHiringManagerFiltered(String manager) {
		...
		for (int i = 1; i <= rows.size(); i++) {
			String dynamicXpath = "//div[@class='oxd-table-body']/div["+i+"]//div[contains(@class,'oxd-table-cell')]//descendant::div[contains(text(),'"+manager+"')]";
	...
	public boolean isStatusFiltered(String status) {
		...
		for (int i = 1; i <= rows.size(); i++) {
			String dynamicXpath = "//div[@class='oxd-table-body']/div["+i+"]//div[contains(@class,'oxd-table-cell')]//descendant::div[contains(text(),'"+status+"')]";
	...
	public boolean isDateOfApplicationWithinRange() {
		  WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
		    ...
		    String dateFromText = candidates_dateFrom.getAttribute("value");
		    String dateToText = candidates_dateTo.getAttribute("value");
		    ...
		    List<WebElement> dateElements = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(
		            By.xpath("//div[@class='oxd-table-body']/div[1]/div[1]/div[5]")));
